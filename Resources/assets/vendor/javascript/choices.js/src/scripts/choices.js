...
class Choices {
  ...
  showDropdown(preventInputFocus) {
    if(this.config.tag === true && this.config.searchEnabled === false) {
      return this;
    }
  ...
  _onKeyDown(event) {
    ...
    if (!this.config.tag && /[a-zA-Z0-9-_ ]/.test(keyString)) {
      this.showDropdown();
    }
  ...
  _onEnterKey({ event, target, activeItems, hasActiveDropdown }) {
    ...
    if (this.config.tag && target.value) {
      const { value } = this.input;
      const canAddItem = this._canAddItem(activeItems, value);
  ...
  _onFocus({ target }) {
    ...
    const focusActions = {
      ...
      [SELECT_MULTIPLE_TYPE]: () => {
        if (target === this.input.element) {
          if(this.config.tag === false || this.config.searchEnabled === true) {
            this.showDropdown(true);
          }
  ...
  _createElements() {
    this.containerOuter = new Container({
      element: this._getTemplate(
        ...
        this._isSelectOneElement,
        this.config.searchEnabled,
        (this.config.tag ? "select-tag" : this.passedElement.element.type ),
      ),
      classNames: this.config.classNames,
